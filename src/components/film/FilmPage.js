...
import {Container, Darken, Footer, Header, Logo, Row, Spinner} from '../layout/index'
...
class FilmPage extends Component {
  ...
  performSearch(){
    search(SEARCH_TYPE.title, this.props.match.params.title)
      .then(movies => {
  ...
  searchRelated() {
    ...
    if (movie && movie.director) {
      ...
      search(SEARCH_TYPE.director, movie.director)
        .then(movies => {
          this.setState({relatedMovies: movies, loadingRelated: false, error: ''})
        }, (err) => {
          this.setState({relatedMovies: [], loadingRelated: false, error: err.message})
        })
    }
  }
